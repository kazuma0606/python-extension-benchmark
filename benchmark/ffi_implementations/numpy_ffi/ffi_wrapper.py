...
class NumPyFFI(FFIBase):
    ...
    def _ensure_library_built(self, library_dir: str):
        """Ensure the NumPy library is built."""
        import platform
        
        # Determine expected library name
        system = platform.system().lower()
        if system == 'windows':
            lib_name = 'libnumpyfunctions.dll'
        elif system == 'darwin':
            lib_name = 'libnumpyfunctions.dylib'
        else:
            lib_name = 'libnumpyfunctions.so'
    ...
    def _build_library(self, library_dir: str, lib_name: str):
        """Build the NumPy shared library."""
        try:
            ...
            if built_files:
                ...
                print(f"Successfully built NumPy FFI library: {lib_name}")
            else:
                print("Warning: NumPy build completed but no shared library found")
                
        except subprocess.CalledProcessError as e:
            print(f"Failed to build NumPy FFI library: {e}")
            print(f"stdout: {e.stdout}")
            print(f"stderr: {e.stderr}")
        except Exception as e:
            print(f"Error building NumPy FFI library: {e}")
        finally:
